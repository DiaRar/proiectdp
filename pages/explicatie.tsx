    ...
    <div className={styles.container}>
      <div className={` ${styles.potato} ${styles.oof}`}>
        ...
        <h3 className={styles.h2}>Explicație</h3>
          <br />
          <span className={styles.mono}>M</span> este un multiplu al numerelor <span className={styles.mono}>A</span> si <span className={styles.mono}>B</span>, deci va fi divizibil cu cel mai mic multiplu comun <span className={styles.mono}>[A, B] = n</span>. Astfel problema se rezumă la a găsi cel mai mic numar format din cifrele 0 și 1, cu proprietatea că acesta este divizibil cu <span className={styles.mono}>n</span>. Generalizând, problema constă în găsirea celui mai mic numar <span className={styles.mono}>x</span> cu proprietatea <span className={styles.mono}>x ≡ k1 (mod k2)</span>, format din cifre aparținând unei mulțimi. Putem reprezenta numerele formate din aceste cifre în ordine crescătoare sub forma unui graf unde există muchie între <span className={styles.mono}>u</span> în <span className={styles.mono}>v</span> dacă <span className={styles.mono}>n<sub>v</sub> = n<sub>u</sub> ⋅ 10 + c</span>, unde <span className={styles.mono}>c</span> aparține mulțimii cifrelor. În consecință, acest graf va fi format din <span className={styles.mono}>k</span> arbori, unde <span className={styles.mono}>k</span> reprezintă cardinalul mulțimii cifrelor nenule. Astfel, în cazul problemei particulare (mulțimea cifrelor este <span className={styles.mono}>{"{"}0, 1{"}"}</span>, <span className={styles.mono}>k1 = 0</span> și <span className={styles.mono}>k2 = n</span>), graful va fi un arbore binar a cărui rădăcina o vom considera nodul reprezentat numărului 1 (format doar din cifra 1). Pentru găsirea celui mai mic multiplu al lui <span className={styles.mono}>n</span> în arbore, vom parcurge graful în ordinea distantei fața de radăcină, vizitând copiii în ordinea crescătoare a numerelor reprezentate de aceștia și marcând nodurile vizitate în funcție de restul obținut prin împărtirea la <span className={styles.mono}>n</span> pentru a nu vizita un număr mai mare (găsit mai târziu) cu același rest. Parcurgerea se termină când găsim nodul al cărui număr reprezentant se divide cu <span className={styles.mono}>n</span>.
          <br />
          <br />
